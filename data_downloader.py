"""
Medical Data Downloader with Content Filtering
Downloads medical datasets from multiple curated sources with strict medical content filtering

Features:
- Medical keyword filtering to ensure only health-related content is kept
- Automatic labeling of sources (credible, misleading, false)
- Non-medical content removal from Kaggle datasets
- Optional Hugging Face / GitHub dataset downloads
- Synthetic + API-friendly hooks for scaling to 50k+ statements
- Statistics on downloaded data

Sources:
1. Kaggle: COVID-19 & health misinformation datasets
2. GitHub/Zenodo: Medical misinformation & fact datasets
3. Hugging Face mirrors (SciFact, PubHealth, etc.)
4. (Web scraping removed to avoid repeated 404s)
"""
...
def is_url_accessible(url):
    """Quick HEAD request to avoid repeated 404 errors."""
    try:
...
# --- 1. Kaggle Dataset Downloader (API) ---
def download_kaggle_datasets(slugs):
    """Downloads and unzips multiple Kaggle datasets using the Kaggle CLI API."""
    # Check if Kaggle API is configured
    kaggle_config_path = os.path.expanduser('~/.kaggle/kaggle.json')
    if not os.path.exists(kaggle_config_path):
        print("   ⚠️  Kaggle API not configured. Skipping Kaggle datasets.")
        print("   (This is OK - other data sources will be used)")
        return
    
    kaggle_slugs_list = [
        # Proven working medical datasets
        ...
        # Health misinformation / fake news datasets (general + medical)
        ...
        for root, dirs, files in os.walk(DATA_DIR):
            for file in files:
                if file.endswith('.csv'):
                    ...
                    try:
                        df = pd.read_csv(file_path, nrows=1000, on_bad_lines='skip')  # Sample first 1000 rows
...
# --- 2. Direct File Downloader (Zenodo / GitHub Releases / HuggingFace mirrors) ---
...
# --- 3. Web Scraper for Credible Sources (deprecated but kept for reference) ---
def scrape_fact_check_page(url, output_file_name, source_name=None, expected_label='credible'):
    """Custom scraper for official fact-checked sources (medical content only)."""
    output_path = os.path.join(DATA_DIR, output_file_name)
    try:
        headers = {
        ...
        if main_content:
            ...
            for element in main_content.find_all(['h1', 'h2', 'h3', 'h4', 'p', 'li', 'div']): 
                ...
                # MEDICAL CONTENT CHECK - Only keep medical-related text
                if is_medical_content(text, min_medical_keywords=2):
                    # Add label metadata for processing
        ...
        if extracted_texts:
            # Add source metadata at the top
...
def save_raw_data_to_csv():
    """Save all downloaded raw data to a CSV file before labeling"""
                                ...
                                if pd.notna(row[col]):
                                    ...
                                    if len(text) >= 10 and is_medical_content(text):
                                        raw_records.append({
                                            ...
                                            'source_type': 'csv',
                                            'label': None  # Will be assigned during labeling
                                        })
                    except Exception as e:
                        print(f"      ⚠ Error reading CSV {file}: {e}")
        
        # Process TXT files (scraped data)
        for root, dirs, files in os.walk(DATA_DIR):
            for file in files:
...
def analyze_downloaded_data():
    """Analyze and report statistics on downloaded data"""
    try:
        total_files = 0
        total_csv = 0
        total_txt = 0
...
if __name__ == '__main__':
    ...
    # 1. KAGGLE DATASETS (General Health)
    print("1. Downloading Kaggle datasets...")
    download_kaggle_datasets([]) 

    # 2. DIRECT URL DOWNLOADS (Multi-Disease Datasets)
    print("\n2. Downloading medical datasets from GitHub/URLs...")
    direct_download_targets = [
        # Medical / misinformation datasets (GitHub repositories)
        ...
        # Medical Research Datasets (embeddings / background)
    ...
    # 3. Hugging Face dataset downloads skipped (auth required)
    ...
    # 4. (Web scraping skipped to avoid repeated 404s)
